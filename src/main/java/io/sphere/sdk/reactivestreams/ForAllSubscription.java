...
final class ForAllSubscription<T extends Identifiable<T>, C extends QueryDsl<T, C>> extends Base implements Subscription {
    ...
    public void request(final long n) {
        if (n < 1) {
            final IllegalArgumentException exception = new IllegalArgumentException("Request amount must be > 0. [0.39]");
    ...
    private void fetchNewElements() {
        ...
        final Query<T> query = lastId == null ? seedQuery : seedQuery.withPredicates(SphereInternalUtils.listOf(seedQuery.predicates(), idIsGreaterThanLastIdPredicate));
