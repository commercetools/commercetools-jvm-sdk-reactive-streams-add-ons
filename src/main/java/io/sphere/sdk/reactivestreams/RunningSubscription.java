...
import static io.sphere.sdk.reactivestreams.ReactiveStreamsLogger.LOGGER;
...
class RunningSubscription<T, C extends QueryDsl<T, C>> extends Base implements SubscriptionsState {
    ...
    public void request(final long n) {
        LOGGER.trace("request({})", n);
        if (n < 1) {
            final IllegalArgumentException exception = new IllegalArgumentException("Request amount must be > 0. [3.9]");
    ...
    private void fetchNewElements(final long initialRemainingElements) {
        ...
        try {
            ...
            LOGGER.error("subscription raised onError", e);
